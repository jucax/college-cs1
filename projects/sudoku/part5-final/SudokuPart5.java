...
public class SudokuPart5
{
    ...
    /**
     * Parameterized constructor that allows the user to create a Sudoku based on the numbers of 
     * one specific text file.
     * 
     * @param sudokuFile - the name of the fime with the sudoku numbers
     */
    public SudokuPart5(String sudokuFile) throws FileNotFoundException {
        //Initialize array, reader and scanner
        this.grid = new int[9][9];
        FileReader reader = new FileReader(sudokuFile);
        Scanner scanner = new Scanner(reader);

        while (scanner.hasNext()) {
            //Iterate inside the 2D array to update the variables that represent rows and columns
            for (int i = 0 ; i < grid.length ; i++) {
                for (int j = 0 ; j < grid[i].length ; j++) {
                    
                    assert scanner.hasNext() : "Not enough values in your sudoku file.";
                    String value = scanner.next();
                    
                    //If we read a "-" in the txt document, we add a 0 to the arrray because we can add String
                    if (value.equals("-")) {
                        grid[i][j] = 0;
                    } else { //If we a number, then we have to convert to number to int and add it to the grid
                        int intValue = Integer.parseInt(value);
                        assert intValue >= 1 && intValue <= 9 : "All numbers must be in the range 1-9.";
                        grid[i][j] = intValue;
                    }
                }
            }
